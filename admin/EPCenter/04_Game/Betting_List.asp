...
<%
	...
	Call dfgameSql.RetrieveInfo_Betting1(dfDBConn.Conn,  page, pageSize, Search, Find, sStartDate, sEndDate, site, reqIB_AMOUNT, real_user, 1)
    'dfGameSql.debug
...
<tr>
    <td>
        <b> <font color="red">**검색할 아이디를 클릭하면 마지막 충전금액에 대한 롤링금액이 표시됩니다</font><br>
	      </b>
    </td>
</tr>
</table>    
...
END IF
%>
<% If Search = "IB_ID" Then %>
<%
	SQLLIST = "SELECT top 1 convert(varchar(20),IC_REGDATE,120), ic_amount FROM INFO_CHARGE WHERE IC_ID='" & IB_ID & "' order by ic_regdate desc "
	SET RSLIST = DbCon.Execute(SQLLIST)

	If isNull(RSLIST(0))  Then
		IC_REGDATE = "충전금이 없습니다"
		ic_amount = 0
	Else
		IC_REGDATE = RSLIST(0)
		ic_amount = FormatNumber(RSLIST(1),0)
	End if

	RSLIST.CLOSE
	SET RSLIST = Nothing

	SQLLIST = "SELECT sum(-lc_cash) FROM log_cashinout with(nolock) WHERE LC_ID='" & IB_ID & "' AND LC_CONTENTS='배팅차감' AND convert(varchar(20),LC_REGDATE,120) > '" & IC_REGDATE & "' "
	SET RSLIST = DbCon.Execute(SQLLIST)

	If isNull(RSLIST(0))  Then
		totalLC_CASH = 0
	ELSE
		totalLC_CASH = FormatNumber(RSLIST(0),0)
	End if

	RSLIST.CLOSE
	SET RSLIST = Nothing

	SQLLIST = "SELECT sum(-lc_cash) FROM log_cashinout with(nolock) WHERE LC_ID='" & IB_ID & "' AND LC_CONTENTS='배팅차감' and lc_liveyn='N' AND convert(varchar(20),LC_REGDATE,120) > '" & IC_REGDATE & "'"
	SET RSLIST = DbCon.Execute(SQLLIST)

	If isNull(RSLIST(0))  Then
		totalspLC_CASH = 0
	ELSE
		totalspLC_CASH = FormatNumber(RSLIST(0),0)
	End If
	...
	If isNull(RSLIST(0))  Then
		totalliLC_CASH = 0
	ELSE
		totalliLC_CASH = FormatNumber(RSLIST(0),0)
	End If
...
%>
<tr><td>
</td><font color="Red"><b><%=iu_nickname%></b></font>님의 마지막 충전후 롤링금액 입니다</br>
충전일시: <%=IC_REGDATE%></br>
충전금액: <font color="Red"><b><%=ic_amount%></b></font>원</br>
총베팅  금액: <font color="Red"><b><%=totalLC_CASH%></b></font>원</br>
스포츠베팅금: <font color="Red"><b><%=totalspLC_CASH%></b></font>원</br>
라이브베팅금: <font color="Red"><b><%=totalliLC_CASH%></b></font>원</br>
