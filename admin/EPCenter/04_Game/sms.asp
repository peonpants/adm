...
<%
	...
	Dim smsVariant
	Set smsVariant = New MVariant			' 클래스 셋팅
	...
	smsVariant.cTranid = "lohas"							' SMS 사용자 ID
    smsVariant.cTranpasswd = "43708"						' SMS 사용자 암호
    smsVariant.cTranphone = request("cTranphone")			' 받는사람 핸드폰 번호
    smsVariant.cTrancallback = request("cTrancallback")		' 보내는 사람 핸드폰 번호
    smsVariant.cTrandate = "0"											' 보낼 날짜 (값이 '0' 일경우 지금당장보내는것, 그 외에 예약 발송일 경우 날짜 패턴은 YYYY-MM-DD HH:MM:SS 이다)
    smsVariant.cTranmsg = trim(request("cTranmsg"))		' 보낼 메세지
