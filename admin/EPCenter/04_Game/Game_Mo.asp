...
<%
	...
	UPDSQL = "UPDATE INFO_GAME SET IG_Status = 'C', IG_Draw = 1 "
	...
	IF NOT RS.EOF THEN
		...
		DO UNTIL RS.EOF
			...
			ELSE
				...
				'// 모든게임이 적중했으면...
				IF ProcFlag = "TRUE" and ResultFlag = "TRUE" THEN
					BenefitAmount = Cdbl(IB_Amount) * int(TotalBenefit*100)/100

					'사용자 당첨금 배당
					UPDSQL = "UPDATE INFO_USER SET IU_Cash = IU_Cash + "&int(BenefitAmount)&" WHERE IU_ID = '" & IB_ID & "' AND IU_SITE = '"& IB_SITE &"'"
					DbCon.execute(UPDSQL)
					...
					INSSQL = "INSERT INTO LOG_CASHINOUT( LC_ID, LC_Cash, LC_GCASH, LC_Contents, LC_SITE) values( '"
					INSSQL = INSSQL & IB_ID & "', "
					INSSQL = INSSQL & int(BenefitAmount) & ","
