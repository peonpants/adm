...
<%
  ...
  '######### Request Check                    ################	    
	...
	Call dfCpSql.RetrieveLOG_LOGIN_FAIL(dfDBConn.Conn,  page, pageSize, sortColumn, sortDirection, Search, Find, sStartDate, sEndDate, site)
...
function whoisSearch(ip) 
{
		...
		document.forms["whois_search_form"].domain_name.value = ip
 
		document.forms["whois_search_form"].target		= "_blank";
		document.forms["whois_search_form"].method	= "post";
		document.forms["whois_search_form"].action	= "http://whois.kisa.or.kr/result.php";
...
<tr>
	...
	<td align="center"  bgcolor="e7e7e7" width=""><b>비밀번호</b></td>
...
<tr bgcolor="#FFFFFF"><td align="center" colspan="6" height="35">로그인 정보가 없습니다.</td></tr>
...
<%
	...
	FOR i = 0 TO dfCpSql.RsCount -1
		...
		LL_NICKNAME		= dfCpSql.Rs(i,"IU_NICKNAME")
		LL_IP			= dfCpSql.Rs(i,"LL_IP")
		LL_REGDATE		= dfCpSql.Rs(i,"LL_REGDATE")
		LL_SITE			= dfCpSql.Rs(i,"IU_SITE")
		LL_PW			= dfCpSql.Rs(i,"LL_PW")
...
<tr  bgcolor="#FFFFFF" >
	<td width="">&nbsp;<a href="/EPCenter/01_CP/Login_List.asp?sStartDate=<%=sStartDate%>&sEndDate=<%=sEndDate%>&Search=LL_ID&Find=<%=LL_ID%>" target="_blank"><%=LL_ID%>

</a></td>
	<td width="" align="center">
	<a href="/EPCenter/01_CP/Login_List.asp?sStartDate=<%=sStartDate%>&sEndDate=<%=sEndDate%>&Search=LL_NICKNAME&Find=<%=LL_NickName%>" target="_blank"><%=LL_NickName%></a></td>
	<td align='center'><%=LL_PW%></td>
	<td align='center'>
	<a href="/EPCenter/01_CP/Login_List.asp?sStartDate=<%=sStartDate%>&sEndDate=<%=sEndDate%>&Search=LL_SITE&Find=<%=LL_SITE%>" target="_blank"><%=LL_SITE%></a></td>
	<td  align="center">
	<% IF BLOCKFLAG = "NO" THEN %>
	<table border="0" cellpadding="0" cellspacing="0">
	<tr><td width="100"><a href="/EPCenter/01_CP/Login_List.asp?sStartDate=<%=sStartDate%>&sEndDate=<%=sEndDate%>&Search=LL_IP&Find=<%=LL_IP%>" target="_blank"><%=LL_IP%></a></td>
