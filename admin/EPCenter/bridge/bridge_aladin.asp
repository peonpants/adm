<META HTTP-EQUIV="Refresh" CONTENT="7;url=http://mainadm.plsfwad.com:26115/EPCenter/bridge/bridge_aladin.asp">
...
<%	
	SQLR = "SELECT top 1 IG_STARTTIME,II_IDX,IG_TEAM1BETTING,IG_TEAM2BETTING from info_game where ig_memo is null and ig_type=0 and ig_anal is null and ig_event='A' and (dateadd(ss,60,ig_starttime) between getdate() and dateadd(ss,50,getdate()))"
...
<%
	SQLR = "SELECT top 1 IG_STARTTIME,II_IDX,IG_TEAM1BETTING,IG_TEAM2BETTING from info_game where ig_memo is null and ig_type=1 and ig_anal is null and ig_event='A' and (dateadd(ss,60,ig_starttime) between getdate() and dateadd(ss,50,getdate()))"
	SET RS = Server.CreateObject("ADODB.Recordset")
	RS.Open SQLR, DbCon, 1
...
<%
	SQLR = "SELECT top 1 IG_STARTTIME,II_IDX,IG_TEAM1BETTING,IG_TEAM2BETTING from info_game where ig_memo is null and ig_type=2 and ig_anal is null and ig_event='A' and (dateadd(ss,60,ig_starttime) between getdate() and dateadd(ss,50,getdate()))"
	SET RS = Server.CreateObject("ADODB.Recordset")
	RS.Open SQLR, DbCon, 1
