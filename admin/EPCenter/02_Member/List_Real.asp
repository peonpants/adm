...
<%
	...
	SITE        = Trim(dfRequest.Value("IU_SITE"))
    IF SITE = "" Then
        SITE = "ALL"
    End IF
	...
	Call dfmemberSql.RetrieveINFO_USER_REAL(dfDBConn.Conn,  page, pageSize, sortColumn, sortDirection, Search, Find, sStartDate, sEndDate, site)
    'dfmemberSql.debug
...
<%  end if %>		  
	...
	<% IF request.Cookies("AdminLevel") = 1 THEN %> 
	    ...
	    <input type="button" value="배팅" onclick="window.open('/EPCenter/04_Game/Betting_List_static.asp?Search=IB_ID&Find=<%= IU_ID %>')" class="input">
