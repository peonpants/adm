...
<%
    ...
    IF reqType = 3 Then
	    Call dfmemberSql.RetrieveLOG_LOGINByDay(dfDBConn.Conn,  reqType, IU_NICKNAME)
    ElseIF reqType = 4 Then
    
        Call dfmemberSql.RetrieveLOG_LOGINByDay(dfDBConn.Conn,  reqType, LL_REGDATE)
...
function loginList()
{
    window.open("/EPCenter/01_cp/login_list.asp?Search=LL_NICKNAME&Find=<%= IU_NICKNAME %>");
}    

function userInfo(IU_IDX)
{
    window.open("/EPCenter/02_Member/View.asp?IU_IDX=" +  IU_IDX);
}    
