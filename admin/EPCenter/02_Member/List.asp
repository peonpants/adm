...
<%
	...
	SearchStatus  = Trim(dfRequest.Value("SearchStatus"))
   ...
   'response.end
	...
	Call dfmemberSql.RetrieveINFO_USER_NEW(dfDBConn.Conn,  page, pageSize, sortColumn, sortDirection, Search, Find, sStartDate, sEndDate, reqIU_SITE,SearchStatus)
    'dfmemberSql.debug
    ...
    if sortColumn = "IU_LOGINDATE" Then sortimg_logdate = sortimg
%>
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function changeLevel(IU_IDX)
    {
        IU_Level = document.getElementById("IU_LEVEL_" + IU_IDX).value;
        
        new Ajax.Request('ajaxLevelUpdate.asp?IU_IDX=' + IU_IDX + "&IU_LEVEL=" + IU_Level ,
        {
            onFailure: function(){alert("레벨 입력 오류");} 
    ...
    function changeDetail(IU_IDX)
    {
        IU_Detail = document.getElementById("IU_DETAIL_" + IU_IDX).value;
        
        new Ajax.Request('ajaxDetailUpdate.asp?IU_IDX=' + IU_IDX + "&IU_DETAIL=" + IU_Level ,
        {
            onFailure: function(){alert("회원상세 입력 오류");} 
            ,onComplete : function(req)
            {                
               alert( req.responseText );
...
function showHideMemo(IU_IDX)
{
    var dis = document.getElementById("trMemo" + IU_IDX).style.display == "none" ? "block" : "none" ;
    document.getElementById("trMemo" + IU_IDX).style.display = dis;
}
</SCRIPT>
</head>
...
<tr>
    <td><b class="text07"> 회원관리 - [총 인원 : <%=TOMEM%> 명] 
...
<form name="MainForm" method="get" action="List.asp">
  <tr>
	...
	<td>
	  <select name="SearchStatus" style="width:100px;font-size:9pt;bgcolor:#F5E0E0;padding-left:10px" class="input">
	    <option value="" <%if SearchStatus = "" then Response.Write "selected"%>>전체</option>
	    <option value="1" <%if SearchStatus = "1" then Response.Write "selected"%>>정상</option>
		<option value="0" <%if SearchStatus = "0" then Response.Write "selected"%>>정지</option>		
		<option value="9" <%if SearchStatus = "9" then Response.Write "selected"%>>탈퇴</option>
		<option value="8" <%if SearchStatus = "8" then Response.Write "selected"%>>삭제</option>
      </select>
	</td>
	<td>
	  <img src="blank.gif" border="0" width="5" height="1">
	...
	<td>
	  <input type="button" value="엑셀저장" onclick="location.href='List_Excel.asp?sStartDate=<%=REQUEST("sStartDate")%>&sEndDate=<%=REQUEST("sEndDate")%>&Search=<%=Search%>&Find=<%=Find&"&Search2="&Search2&"&Search6="&Search6&"&Search5="&Search5%>';" style="border: 1 solid; background-color: #C5BEBD;" id=button1 name=button2>
	</td>
	<% END IF %>
  </tr>
  </form>
...
<col width="70" />
  ...
  <tr bgcolor="e7e7e7">
	...
	<!--
	<td align="center"  ><b>
	  <%Response.Write "<a href=List.asp?page="&i&"&sStartDate="&REQUEST("sStartDate")&"&sEndDate="&REQUEST("sEndDate")&"&Search="& Search&"&Find="&Find&"&Search2="&Search2&"&Search5=vip>No</td>"
	  %>
    -->	  	
	<td align="center"  >
	  <b>아이디</b>
	...
	<td align="center"  >
	  <b><a href="List.asp?sortColumn=IU_REGDATE&sortDirection=<%= sortDirection1 %>">등록일<%= sortimg_date %></a></b></br>
	  <a href="List.asp?sortColumn=IU_LOGINDATE&sortDirection=<%= sortDirection1 %>">최근접속일<%= sortimg_logdate %></a></b>
	</td>
	<td align="center"  >
	  <b>가입아이피<br>마지막 접속 아이피</b>	  
	</td>	
	<td align="center"  >
	  <b>사이트</b>
	</td>
...
ELSE
	...
	FOR i = 0 TO dfMemberSql.RsCount -1
		...
		IU_Ip			= dfMemberSql.Rs(I,"IU_IP")	
		IU_Status		= dfMemberSql.Rs(I,"IU_STATUS")
		IU_SITE			= dfMemberSql.Rs(I,"IU_SITE")
		IU_SSITE		= dfMemberSql.Rs(I,"IU_SSITE")
		IU_GROUP		= dfMemberSql.Rs(I,"IU_GROUP")
		...
		IU_DETAIL    = dfMemberSql.Rs(I,"IU_DETAIL")
		...
		IF IU_IP = null or IU_IP = "" then
			IU_IP = "<font color=red>관리자가입</font>"
		END IF

		IF ll_ip = null or ll_ip = "" then
			ll_ip = "<font color=red>미접속</font>"
		END IF
		...
		Else
		    trColor = "#F0F0F0"
		End If
		IF IU_LOGINDATE = null or IU_LOGINDATE = "" then
			IU_LOGINDATE = "<font color=red>미접속</font>"
		Else
			IU_LOGINDATE = right(dfStringUtil.GetFullDate(IU_LOGINDATE),14)
		END IF
%>
...
%>
    <td bgcolor="#sbfcgt">         
<%  end if %>		  
        <b><a href="View.asp?IU_IDX=<%=IU_IDX%>&IU_SITE=<%=IU_SITE%>&PAGE=<%=PAGE%>"><%=iu_id%></a></b>
	</td>
	
	<td align="center">
	    <input type="text" name="IU_LEVEL_<%= IU_IDX %>"  id="IU_LEVEL_<%= IU_IDX %>" value="<%= IU_LEVEL %>" size="2" maxlength="2" class="input" onblur="changeLevel(<%= IU_IDX %>);" />
	</td>
	...
	<td align="center">
	  <%=right(dfStringUtil.GetFullDate(IU_RegDate),14)%></BR>
	  <%=IU_LOGINDATE%>
	</td>
	<td align="center">
	<%
	if IU_IP = "<font color=red>관리자가입</font>" THEN
	%>
		<%=IU_IP%>
	<%
	ELSE
	%>
	  <A href="../01_CP/Login_List.asp?sStartDate=&sEndDate=&Search=LL_IP&Find=<%=iu_ip%>"><%=iu_ip%></a>
	<%
	END IF
	%>
	<br>
	<%
	if ll_ip = "<font color=red>미접속</font>" THEN
	%>
		미접속
	<%
	ELSE
	%>
	  <A href="../01_CP/Login_List.asp?sStartDate=&sEndDate=&Search=LL_IP&Find=<%=ll_ip%>"><%=ll_ip%></a>
	<%
	END IF
	%>
	</td>
	...
	<td>
	<% IF request.Cookies("AdminLevel") = 1 THEN %> 
	    <input type="button" value="배팅" onclick="location.href='/EPCenter/04_Game/Betting_List.asp?Search=IB_ID&Find=<%= IU_ID %>'" class="input">
	...
	<td align="center">
<% If IU_DETAIL <> "" Then %>
    <a href="javascript:showHideMemo(<%= IU_IDX %>)">보기</a>
<% End If %>
	</td>
  </tr>
  <tr  id="trMemo<%= IU_IDX %>" style="display:none;">
    <td colspan="22" bgcolor="#FFFFFF">
    <table width="100%">
    <tr>
        <td width="80">상세정보</td>
        <td width="700">
            <input type="text" name="IU_DETAIL" value="<%= IU_DETAIL %>" class="input" style="width:700px" readonly />
        </td>              
    </tr>   
    </table>
    </td>
