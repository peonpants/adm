...
<%
	...
	dfDBConn.Connect()	
Response.Write "<script>alert('" & LAC_GROUP1 & "');</script>" 
Response.Write "<script>alert('" & LAC_GROUP2 & "');</script>" 
Response.Write "<script>alert('" & LAC_GROUP3 & "');</script>" 
	'######### 관리자 접속 로그 부름                    ################	
    Call dfCpSql.RetrieveINFO_ADMIN_L3(dfDBConn.Conn, LAC_GROUP,LAC_GROUP1,LAC_GROUP2,LAC_GROUP3)
...
<form name="MainForm" action="/EPCenter/05_Account/CashCalMonth_L3.asp" method="post" align="center">
시작일자 :
 ...
 <table  border="0" cellspacing="1" cellpadding="5" bgcolor="#AAAAAA" width="100%">
		...
		<%		
            ...
            IF dfCpSql.RsCount <> 0 Then
				...
				SQL = "SELECT sum(IB_IDX) as charge FROM LOG_ADMIN_CASHINOUT WHERE LAC_TYPE=1 and LAC_GROUP=2 and LAC_GROUP1 = " & LAC_GROUP1 & " and LAC_GROUP2 = " & LAC_GROUP2 & " and LAC_GROUP3 = " & LAC_GROUP3 & " and LAC_REGDATE>='"&sStartDate&"' AND LAC_REGDATE<='"&sEndDate&"'"
				...
				SQL = "SELECT sum(IB_IDX) as excharge FROM LOG_ADMIN_CASHINOUT WHERE LAC_TYPE=3 and LAC_GROUP=2 and LAC_GROUP1 = " & LAC_GROUP1 & " and LAC_GROUP2 = " & LAC_GROUP2 & " and LAC_GROUP3 = " & LAC_GROUP3 & " and LAC_REGDATE>='"&sStartDate&"' AND LAC_REGDATE<='"&sEndDate&"'"
				SET RS1 = DbCon.Execute(SQL)
