...
<tr><td align="center" height="30" bgcolor="e7e7e7" width="60"><b><input type="Checkbox" name="chkAll" onclick="AllChk();"></b></td>
	...
	<td align="center" height="30" bgcolor="e7e7e7"><b>레벨</b></td>
...
<%
	...
	FOR i = 0 TO dfcustomerSql.RsCount -1 
		...
		SQLMSG = "SELECT IU_LEVEL from info_user with(nolock) WHERE IU_ID = '" & BC_ID &"'"
		SET RS = DbCon.Execute(SQLMSG)

		IF Not RS.EOF Then
			BC_LEVEL = RS("IU_LEVEL")
		End If
...
<tr <% IF BC_Status <> "1" THEN Response.Write "bgcolor='#d6f7fd'" Else Response.Write "bgcolor='#ffffff'" End IF %>>
	...
	Else 
	%>
	<td>&nbsp;<b><a href="/EPCenter/07_Customer/List.asp?Search=BC_Writer&Find=<%=BC_Writer%>&BC_Type=<%= BC_Type%>"><%=BC_ID%></b>&nbsp;&nbsp;(<%=BC_Writer%>)</td>
	<%
	End If 
	%>
	<td align="center"><%=BC_LEVEL%></td>
	<td align="center">
