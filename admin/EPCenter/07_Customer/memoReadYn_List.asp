...
<%
	...
	DbCon.execute SQL
'    '######## 안 읽은 문의 표시 ################
'	SQLLIST = "delete from board_customer_template where bct_title='1'"
'
...
'	SET RSLIST = Nothing
'	'TMSG_2 = "10247"
'	If TMSG_2 <> "" And TMSG_2 <> "0" Then
%>
<!--<script type="text/javascript">location.href="View.asp?BC_Idx=<%=TMSG_2%>";</script>-->
<%
'	End If 
%>
<%
	...
	Call dfcustomerSql.RetrieveBoard_MemoCustomer(dfDBConn.Conn,  page, pageSize, Search, Find, sStartDate, sEndDate , BC_Type)
    'dfcustomerSql.debug
...
<tr>
    <td><b class="text07"> 쪽지 확인 여부</b></td>
</tr>
</table>    
...
<tr>
	...
	<td align="center" height="30" bgcolor="e7e7e7"><b>수신인</b></td>
	<td align="center" height="30" bgcolor="e7e7e7"><b>사이트</b></td>
	<td align="center" height="30" bgcolor="e7e7e7" width="100"><b>작성일</b></td>
	<td align="center" height="30" bgcolor="e7e7e7" width="60"><b>확인여부</b></td>
...
<tr <% IF BC_Status <> "1" THEN Response.Write "bgcolor='#d6f7fd'" Else Response.Write "bgcolor='#ffffff'" End IF %>>
	...
	<td align="center"><% if BC_READYN = 0 then %>미확인<%else%>확인<%end if%></td>
