...
<%
	Public Function Get_Content_Number(gDBc,gRs)
		Dim set_number, gCmd
		Dim gSql : gSql = "select BOARD_GATE_IDX.NEXTVAL from dual"
		...
		Get_Content_Number = set_number
	End Function 
%>
<%
	upath = "G:\_drama\wwwAdmin\UpFile\Upload"
	IG_SITE = "Truepo"
	...
	If Not oRs.EOF Then
		...
		End If 
			...
			SQL = "INSERT INTO BOARD_GATE ( BF_Title, BF_Contents, BF_Writer, BF_PW, BF_HITS, BF_Level, BF_SITE, BF_REGDATE) VALUES ( ?, empty_clob(), ?, '1111', ?, 0, 'parao24',CAST('"&oRs(2)&" ', '"&xls_time&"') )"
		...
		If Not content_body = "" Then 
			gSql = "UPDATE BOARD_GATE SET BF_Contents = ? WHERE BF_IDX = ? "
	...
	ElseIf oRs(4) <> "" And oRs(2) <> "" Then
		...
		SQL = "INSERT INTO BOARD_GATE_REPLY (  BF_IDX, BFR_CONTENTS, BFR_WRITER, BFR_REGDATE ) VALUES (  ?, ?, ?, CAST('"&oRs(2)&" ', '"&xls_time&"'))"
	...
	SQL = "select count(*) as cnt from BOARD_GATE where bf_site = 'parao24'"
	...
	For i = 1 To  totalCnt
		...
		Sql = " select count(*) as cnt from BOARD_GATE_reply where bf_idx = "&i
		Set gRs = DbCon.Execute (SQL)
			Cnt = gRs("cnt")
		gRs.close
		Set gRs = Nothing
	
		Sql = "update BOARD_GATE set bf_replycnt = "& Cnt & " where bf_idx = "& i
		DbCon.Execute (SQL)
