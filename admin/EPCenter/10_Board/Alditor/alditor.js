...
/////////// 사용자 설정 시작 ////////////
	function alditorConfig () {
		...
		*/
		this.uploadLink = 'editorUpload.asp';
		...
		*/
		...
		this.toolbarItems.push("emoticon2"); // 이모티콘 삽입
...
/////////// 설정 끝 //////////// 이하 손델 필요 없음 ///////
	...
	alditor.prototype.alditorToolbar = function (startNo) {
		...
		for (var i = startNo; i < config.toolbarItems.length; i++)
		{
			switch (config.toolbarItems[i].split("_")[0])
			{
				...
				case "emoticon2":
					this.createButton(config.alditorPath+"buttons/" + config.buttonSet + "/emoticon2.gif", "이모티콘 삽입", "emoticon2");
				break;
	...
	alditor.prototype.toolbarClick = function(action, buttonObj) 
	{
		...
		switch (action)
		{
			...
			case "emoticon2":
				popWin(config.alditorPath + 'alditorEmoticons2.html?id='+this._iframe.id + '&rnd=' + new Date().getTime(), 540,460, actionIframe);
			break;
