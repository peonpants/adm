...
<form name="MainForm" method="get" action="Money_Addadmin_exchange.asp">
...
<tr bgcolor="#FFFFFF" ><td align="center"><%=LAC_IDX%></td>
	<td>&nbsp;<a href="/seller/05_Account/Money_Addadmin_exchange.asp?sStartDate=<%=Left(sStartDate,10)%>&sEndDate=<%=Left(sEndDate,10)%>&Search=LAC_ID&Find=<%=LAC_ID%>"><%=LAC_ID%></a> (<font color="#6666bb"><b><%=IU_NICKNAME%></b></font>)</td>	
	<td align="center"><%=charge%></td>
	<td align="right"><%=FORMATNUMBER(IB_IDX,0)%>원</td>
	<td align='center'><a href="/seller/05_Account/Money_Addadmin_exchange.asp?sStartDate=<%=Left(sStartDate,10)%>&sEndDate=<%=Left(sEndDate,10)%>&Search=LAC_SITE&Find=<%=LAC_SITE%>"><%=LAC_SITE%></a> (<font color="#6666bb"><b><%=IA_NICKNAME%></b></font>)</td>
