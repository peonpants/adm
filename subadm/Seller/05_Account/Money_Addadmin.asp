...
<tr bgcolor="#FFFFFF" ><td align="center"><%=LAC_IDX%></td>
	<td>&nbsp;<a href="/seller/05_Account/Money_Addadmin.asp?sStartDate=<%=Left(sStartDate,10)%>&sEndDate=<%=Left(sEndDate,10)%>&Search=LAC_ID&Find=<%=LAC_ID%>"><%=LAC_ID%></a> (<font color="#6666bb"><b><%=IU_NICKNAME%></b></font>)</td>	
...
<% End If %>
	...
	<td align='center'><a href="/seller/05_Account/Money_Addadmin.asp?sStartDate=<%=Left(sStartDate,10)%>&sEndDate=<%=Left(sEndDate,10)%>&Search=LAC_SITE&Find=<%=LAC_SITE%>"><%=LAC_SITE%></a> (<font color="#6666bb"><b><%=IA_NICKNAME%></b></font>)</td>
