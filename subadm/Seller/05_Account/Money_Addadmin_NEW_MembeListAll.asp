...
if IA_LEVEL < 3 then
	if Group2 = 0 then
	    ...
	    SQL = "SELECT IU_IDX, iu_id, IU_LEVEL, IU_NickName, "
		SQL = SQL & " (select sum(lc_cash) FROM log_cashinout WHERE lc_id = I.iu_id AND lc_regdate > '"& sStartDate &"' AND lc_regdate <  '"& sEndDate &"' AND lc_contents = '배팅차감') AS IU_CASH, "
		SQL = SQL & " IU_POINT, "
		SQL = SQL & " (select sum(lc_cash) FROM log_cashinout WHERE lc_id = I.iu_id AND lc_regdate > '"& sStartDate &"' AND lc_regdate <  '"& sEndDate &"' AND lc_contents = '머니충전') AS IU_CHARGE, "
		SQL = SQL & " (select sum(lc_cash) FROM log_cashinout WHERE lc_id = I.iu_id AND lc_regdate > '"& sStartDate &"' AND lc_regdate <  '"& sEndDate &"' AND lc_contents = '환전차감') AS IU_EXCHANGE, "
		SQL = SQL & " (select count(*) FROM log_login WHERE ll_id = I.iu_id AND ll_regdate > '"& sStartDate &"' AND ll_regdate <  '"& sEndDate &"') AS IU_LOGIN_CNT,  "
		SQL = SQL & " (select count(*) FROM info_betting WHERE ib_id = I.iu_id AND IB_REGDATE > '"& sStartDate &"' AND IB_REGDATE <  '"& sEndDate &"') AS IU_BETCNT, "
		SQL = SQL & " IU_RegDate, IU_SITE, IU_Status   "
		SQL = SQL & " FROM INFO_USER AS I INNER JOIN INFO_ADMIN AS A ON I.iu_site = A.ia_id "
		SQL = SQL & " WHERE A.ia_group1="&Group1&" "& addAndS &"  ORDER BY IU_SITE, A.ia_level desc , IU_ID"
...
<div class="MenuLeft_MainDiv2 width100pIMP">
	<div class="MenuLeft_MainInnerDiv width95pIMP">
		<div style="height:10px;"></div>
			<div style="padding:5px;">
				<div class="panel panel-default">
					<div class="panel-heading"><span class="txtsh011b" style="color:#adc;"> ▶ </span><span style="font-size:18px; font-weight:bold;text-shadow:0px 2px 2px #fff;">기간별 총가입 회원 리스트(기간내 배팅 내용) </span></div>
					<div class="panel-body"> 
						<span style="color:#9999f0">
						*총회원에서 해당기간의 배팅머니,입출금, 로그인,배팅수를 보여줍니다.<br/>
						*많은 인원수, 긴 기간을 선택하시면 속도가 많이 느려집니다.
						</span><br/>

						<script type="text/javascript" src="../includes/calendar1.js"></script>
						<script type="text/javascript" src="../includes/calendar2.js"></script>
						<form name="MainForm" action="Money_Addadmin_NEW_MembeListAll.asp?g1=<%=Group1%>&g2=<%=Group2%>&g3=<%=Group3%>&g4=<%=Group4%>" method="post" align="center">
						시작일자 :
						...
						<button class="btn btn-info btn-xs" type="submit" title="좌측 날짜로 가입회원을 검색합니다.">검색</button>
						<div style="height:10px;"></div>
					</div>
        ...
        %>
			  ...
			  <td class="aliRight" >
						<%If Not isNull(Results(4,i)) Then
							IU_CASH = -Results(4,i)
			  ...
			  <td align="right">
						<%If Not isNull(Results(6,i)) Then
							IU_CHARGE = Results(6,i)
			  ...
			  <td align="right" >
						<%If Not isNull(Results(7,i)) Then
							IU_EXCHANGE = -Results(7,i)
				...
				<% If IU_CHARGE = 0 then%>
					<td align="right">
				<%else%>
					<td align="right" bgcolor="#FfEfEf">
					<% if IU_CHARGE < IU_EXCHANGE then%> <span style="color:red; font-weight:bold"><%end if%>
			  ...
			  <td align="center" >
					<%
						IU_LoginCount = Results(8,i)
						If IU_LoginCount = 0 then
						%><span style="color:#aaa;"><%=IU_LoginCount%></span>
						<%
						Else
						%><%=IU_LoginCount%>
						<%
			  ...
			  <td align="center" >
			  <%
						IU_betCount = Results(9,i)
						If IU_betCount = 0 then
						%><span style="color:#aaa;"><%=IU_betCount%></span>
						<%
						Else
						%><%=IU_betCount%>
						<%
