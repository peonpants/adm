...
<%
	...
	Call dfmemberSql.RetrieveINFO_USERByRecomID(dfDBConn.Conn)
...
<tr>
    <td><b class="text07">  회원관리 ▶ 추천 회원 리스트 > 추천인 회원수 : <%= dfMemberSql.RsCount %> 명 / 추천가입수 <%=  ALLCNT %>명</b></td>
</tr>
</table>    
...
ELSE
	...
	<td>
        <a href="recom_list1.asp?IU_ID=<%= IU_ID %>" target="_blank">[보기]</a>
	</td>
	<td align="center">
	<a href="recom_list1.asp?IU_ID=<%= IU_ID %>" target="_blank"><%=CNT%></a>
...
<%  end if %>		  
	...
	<% IF request.Cookies("AdminLevel") = 1 THEN %> 
	    <input type="button" value="보내기" onclick="location.href='Write_Message.asp?cd=<%=IU_ID%>&cdi=<%=IU_IDX%>&JOBSITE=<%=IU_SITE%>'"> 
	<% ELSE %>
	    -
	<% END IF %> 
	</td>
